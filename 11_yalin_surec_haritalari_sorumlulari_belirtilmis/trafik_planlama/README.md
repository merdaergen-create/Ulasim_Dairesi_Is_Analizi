...
```mermaid
flowchart TD
    A([Plan/revizyon ihtiyacı<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]) --> B[Ulaşım Planlama: kapsam ve çalışma programı<br/>Sorumlu: Ulaşım Planlama Şube Müdürü]
    B --> C[Veri ihtiyaç planı ve veri sorumluları<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    C --> D[AUS: sensör, kamera ve veri platformu çıktıları<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    C --> E[Toplu Taşıma: hat, yolcu, filo ve hizmet verileri<br/>Sorumlu: Trafik Güvenliği ve Sürdürülebilir Ulaşım Uzmanı]
    C --> F[İmar/Fen: arazi kullanımı ve yatırım projeleri<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    D --> G[Veri kalite ve eksiklik kontrolü<br/>Sorumlu: Trafik Etüt ve Veri Analizi Uzmanı]
    E --> G
    F --> G
    G --> H{Veri yeterli mi?<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu}
    ...
    K --> L[Paydaş ve kurum görüşleri<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    L --> M{Teknik ve mali olarak uygulanabilir mi?<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu}
    M -- Hayır --> K
    M -- Evet --> N[Tercih edilen plan, yatırım programı ve KPI<br/>Sorumlu: Ulaşım Planlama Şube Müdürü]
    N --> O[Yetkili kurul/UKOME/Meclis süreci<br/>Sorumlu: UKOME Şube Müdürlüğü ilgili yalın pozisyonu]
    O --> P{Onaylandı mı?<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu}
    P -- Hayır --> Q[Gerekçe ve revizyon<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    Q --> N
    P -- Evet --> R[Uygulama portföyü ve sorumlu birimler<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    R --> S[Periyodik izleme ve plan revizyonu<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    S --> T([Onaylı plan ve izleme raporu<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu])
```
...
```mermaid
flowchart TD
    A([Talep, kaza veya kapasite sorunu<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]) --> B[Planlama Servisi: ön inceleme<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    B --> C{Ulaşım Dairesi yetki alanında mı?<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu}
    C -- Hayır --> D[Yetkili kuruma yönlendirme ve kayıt kapatma<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    ...
    H --> I{Teknik çözüm uygun mu?<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu}
    I -- Hayır --> F
    I -- Evet --> J{UKOME kararı gerekli mi?<br/>Sorumlu: UKOME Şube Müdürlüğü ilgili yalın pozisyonu}
    J -- Evet --> K[UKOME teknik teklif dosyası<br/>Sorumlu: UKOME Şube Müdürlüğü ilgili yalın pozisyonu]
    K --> L[UKOME kararı<br/>Sorumlu: UKOME Şube Müdürlüğü ilgili yalın pozisyonu]
    J -- Hayır --> M[Yetkili teknik onay<br/>Sorumlu: Ulaşım Planlama Şube Müdürü]
    L --> N[Uygulama paketi ve iş programı<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    M --> N
    N --> O[Fen İşleri: yol gövdesi/yapım<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    N --> P[Ulaşım Planlama: sinyal ve işaretleme<br/>Sorumlu: Sinyalizasyon Proje ve İşletme Sorumlusu]
    O --> Q[Ortak saha kontrolü<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    P --> Q
    Q --> R{Proje ve kalite kriterlerine uygun mu?<br/>Sorumlu: Teknik Proje Kontrol Mühendisi}
    R -- Hayır --> S[Düzeltici iş emri<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    S --> Q
    R -- Evet --> T[CBS envanteri, as-built ve kabul<br/>Sorumlu: CBS ve Ulaşım Envanteri Uzmanı]
    T --> U([Proje kapanışı<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu])
```
...
```mermaid
flowchart TD
    A([Başvuru<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]) --> B[E-başvuru ve belge tamlık kontrolü<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    B --> C{Belgeler tam mı?<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu}
    C -- Hayır --> D[Eksik belge bildirimi<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    D --> B
    C -- Evet --> E[Planlama: yetki alanı ve mevzuat kontrolü<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    E --> F[Saha incelemesi ve trafik güvenliği analizi<br/>Sorumlu: Trafik Güvenliği ve Sürdürülebilir Ulaşım Uzmanı]
    F --> G[İmar, altyapı, AYKOME, kolluk ve ilgili kurum görüşleri<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    G --> H{Teknik olarak uygun mu?<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu}
    H -- Hayır --> I[Gerekçeli ret veya proje revizyon talebi<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    I --> J([Süreç kapatılır veya yeniden başvuru<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu])
    H -- Evet --> K{UKOME kararı gerekli mi?<br/>Sorumlu: UKOME Şube Müdürlüğü ilgili yalın pozisyonu}
    K -- Evet --> L[UKOME gündem ve karar süreci<br/>Sorumlu: UKOME Şube Müdürlüğü ilgili yalın pozisyonu]
    K -- Hayır --> M[Yetkili idari/teknik onay<br/>Sorumlu: Ulaşım Planlama Şube Müdürü]
    L --> N[İzin ve trafik tedbir şartlarının yayımlanması<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    M --> N
    N --> O[Başlangıç öncesi saha güvenlik kontrolü<br/>Sorumlu: Saha Kalite, Kabul ve İSG Teknikeri]
    O --> P[Çalışma süresince denetim<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    P --> Q{Süre/şart ihlali var mı?<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu}
    Q -- Evet --> R[Düzeltme, durdurma veya ilgili makama bildirim<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    R --> P
    Q -- Hayır --> S[Çalışma bitişi ve yolun eski hale getirilmesi<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    S --> T[Saha kabulü, fotoğraf ve kapanış kaydı<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    T --> U([Süreç tamamlandı<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu])
```
...
```mermaid
flowchart TD
    A([Yeni ihtiyaç veya mevcut kavşak problemi<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]) --> B[Trafik mühendisliği ve sayım<br/>Sorumlu: Trafik Etüt ve Veri Analizi Uzmanı]
    ...
    E --> F[AUS/Bilgi İşlem: ağ ve merkez entegrasyon tasarımı<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    F --> G[UKOME/kurum onayları<br/>Sorumlu: UKOME Şube Müdürlüğü ilgili yalın pozisyonu]
    G --> H[Tedarik ve saha kurulum planı<br/>Sorumlu: UKOME Şube Müdürlüğü ilgili yalın pozisyonu]
    ...
    L -- Hayır --> M[Hata giderme<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    M --> K
    ...
    Q --> R{Saha ekipmanı mı, merkez/ağ mı?<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu}
    R -- Saha --> S[Sinyalizasyon Servisi müdahalesi<br/>Sorumlu: Sinyalizasyon Proje ve İşletme Sorumlusu]
    R -- Merkez/Ağ --> T[AUS/Bilgi İşlem müdahalesi<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    S --> U[Test ve servis kapanışı<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    T --> U
    U --> V[Veriyle süre optimizasyonu<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
```
...
flowchart TD
    ...
    B --> C{Onaylı proje/karar var mı?<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu}
    C -- Hayır --> D[Planlama ve onay sürecine yönlendirme<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    C -- Evet --> E[Risk, yol sınıfı ve aşınmaya göre öncelik<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    E --> F[Yıllık/aylık iş programı<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    ...
    I --> J{Uygun mu?<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu}
    J -- Hayır --> K[Düzeltme/yenileme<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    K --> I
    J -- Evet --> L[Fotoğraf, metraj ve CBS güncellemesi<br/>Sorumlu: CBS ve Ulaşım Envanteri Uzmanı]
    L --> M[Periyodik performans ve aşınma izlemesi<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
```
...
flowchart LR
    ...
    B --> C{Olay türü<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu}
    ...
    F --> H
    G --> H
    H --> I[Olay süresince izleme<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
    I --> J{Normal işletime dönüldü mü?<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu}
    J -- Hayır --> I
    J -- Evet --> K[Olay kapanışı ve müdahale kayıtları<br/>Sorumlu: Ulaşım Planlama ilgili yalın pozisyonu]
