...
```mermaid
flowchart TD
    A([Trafik güvenliği veya yönetim ihtiyacı<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]) --> B[Ulaşım Planlama: veri, saha ve fizibilite analizi<br/>Sorumlu: Ulaşım Veri Platformu ve Analitik Sorumlusu]
    ...
    C -- Evet --> E[Saha listesi, iş gereksinimi ve beklenen etki<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    E --> F[Kolluk, UKOME ve hukuk görüşleri<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    F --> G{Yetki ve mevzuat uygun mu?<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu}
    ...
    Q --> R[Envanter, konfigürasyon, bakım ve işletme devri<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    R --> S[Planlama: 30/90/180 günlük etki analizi<br/>Sorumlu: Ulaşım Veri Platformu ve Analitik Sorumlusu]
    S --> T([Etki ve performans raporu<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu])
```
...
```mermaid
flowchart TD
    A([Yeni veri kaynağı veya analiz ihtiyacı<br/>Sorumlu: Ulaşım Veri Platformu ve Analitik Sorumlusu]) --> B[İş sahibi şube: amaç ve karar ihtiyacı<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    ...
    E --> F{Veri alınabilir mi?<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu}
    F -- Hayır --> G[Anonimleştirme, kapsam azaltma veya hukuki düzenleme<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    G --> E
    F -- Evet --> H[ETL/API ve veri sözlüğü<br/>Sorumlu: Ulaşım Veri Platformu ve Analitik Sorumlusu]
    H --> I[Doğruluk, tamlık, güncellik ve tekillik kontrolleri<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    I --> J{Kalite eşiği sağlandı mı?<br/>Sorumlu: Haberleşme, Ağ ve Siber Güvenlik Mühendisi}
    J -- Hayır --> K[Kaynak sistem düzeltmesi ve veri kalite görevi<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    K --> I
    ...
    M --> N[İş sahibi şube tarafından anlam ve sonuç doğrulaması<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    N --> O{Karar için uygun mu?<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu}
    O -- Hayır --> M
    O -- Evet --> P[Yayımlama ve periyodik kalite izleme<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
```
...
```mermaid
flowchart TD
    A([Onaylı durak veya dijital iyileştirme ihtiyacı<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]) --> B[Toplu Taşıma: gösterilecek hizmet bilgisi<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    B --> C[Ulaşım Planlama: fiziksel yer ve erişilebilirlik<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    ...
    J -- Hayır --> K[Düzeltme<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    K --> I
    J -- Evet --> L[Durak varlık kaydı ve hizmete alma<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    ...
    N -- Fiziksel --> O[Ulaşım Planlama/Fen bakım<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    N -- Dijital --> P[AUS/yüklenici bakım<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    N -- Veri --> Q[Toplu Taşıma/AUS veri düzeltme<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
...
```mermaid
flowchart TD
    A([Görüntü talebi<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]) --> B[Başvuru kaydı, talep sahibi ve hukuki dayanak<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    B --> C[Yetkilendirilmiş birim/Hukuk: talep yetki kontrolü<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    C --> D{Talep hukuken uygun mu?<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu}
    D -- Hayır --> E[Gerekçeli ret ve kayıt<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    D -- Evet --> F[AUS: kamera, konum ve saklama süresi kontrolü<br/>Sorumlu: AUS Sistem Mimarı ve Entegrasyon Sorumlusu]
    F --> G{Görüntü mevcut mu?<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu}
    G -- Hayır --> H[Mevcut değil/saklama süresi dolmuş cevabı<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    G -- Evet --> I[Yetkili personel tarafından arama ve dışa aktarma<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    I --> J[Bütünlük doğrulaması, maskeleme ve dosya hash<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    ...
    L --> M[Saklama/imha takvimi<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
```
...
```mermaid
flowchart LR
    A[Alarm, kullanıcı ihbarı veya değişiklik talebi<br/>Sorumlu: AUS Sistem Mimarı ve Entegrasyon Sorumlusu] --> B[Kayıt ve etki/öncelik<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    ...
    F --> G[Kök neden ve tekrar önleme<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    C -- Değişiklik --> H[İş sahibi, kapsam, risk ve geri dönüş planı<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
    H --> I[Test ortamı ve kullanıcı kabulü<br/>Sorumlu: Ulaşım Teknolojileri Proje Yöneticisi]
    I --> J{Onaylandı mı?<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu}
    J -- Hayır --> H
    J -- Evet --> K[Kontrollü canlıya alma<br/>Sorumlu: AUS ve TKM ilgili yalın pozisyonu]
